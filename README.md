...
* [Python3.14 64位](https://www.python.org/downloads/)

**注意: 推荐统一采用64位编译，并选择Release配置运行**

关于以上工具的安装和详细配置请参见它们的帮助文档。

### 设置环境变量
在Windows系统环境变量中添加如下4个环境变量，根据Python3.14的安装路径，例如
* PYTHON_PATH  x:\Python314\
* PYTHON_LIB  x:\Python314\libs\
* PYTHON_VERSION  314
* QTDIR 你自己的Qt6安装目录，一定是这样的 X:\Qt6\6.11.1\msvc2022_64\bin
x表示你实际安装的磁盘

## 部署,x64为例
在开始菜单中找到Qt 6.11.1 (MSVC 2022 64-bit)命令行窗口进入构建的目录。
例如：build\...\bin\x64\
再执行以下命令
...
* 找到第37行，根据本地实际输出的构建目录替换掉路径 build\...\bin\x64
* 再执行Build>>Compile(Ctrl+F9)或Run(F9)

运行
-------------------------------------------------------------------------------
在试用运行之前，一般还需要以下几个操作：
* 手动复制3part/PythonQt/lib/PythonQt-Qt6-Python3.14.dll。PythonQt-Qt6-Python3.14.dll是通过PythonQt生成的库。
